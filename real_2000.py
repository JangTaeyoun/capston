...
# -----------------------------
# 5. SCN Layer (희소/밀집 adj 지원) + [수정] EdgeDrop
# -----------------------------
class SourceBasedSCNLayer(nn.Module):
    # [수정] dropout 0.4 -> 0.5, edge_drop (유지)
    def __init__(self, in_dim, out_dim, dropout=0.5, edge_drop=0.2):
        super().__init__()
        self.linear = nn.Linear(in_dim, out_dim)
        self.dropout = nn.Dropout(dropout)
        self.norm = nn.LayerNorm(out_dim)
        self.edge_drop = edge_drop # [유지]
    ...
    def forward(self, x, adj):
        ...
        if hasattr(adj, "is_sparse") and adj.is_sparse:
            ...
            # [유지] EdgeDrop (Train 시에만 적용)
            vals = adj.values()
            if self.training and self.edge_drop > 0:
                m = (torch.rand_like(vals) > self.edge_drop).float()
                # 기댓값 보존을 위한 스케일링
                vals = vals * m * (m.numel() / m.sum().clamp(min=1.))
            
            # EdgeDrop이 적용된 adj
            adj_w_drop = torch.sparse_coo_tensor(adj.indices(), vals, adj.size(), device=adj.device)
            # ---
            
            if adj_w_drop.dtype != torch.float32:
                 adj_w_drop = adj_w_drop.float()
                 
            orig_dtype = x.dtype
            if x.is_cuda:
                with torch.cuda.amp.autocast(False):
                    support = torch.sparse.mm(adj_w_drop, x.float())
            else:
                support = torch.sparse.mm(adj_w_drop, x.to(torch.float32))
...
class SourceBasedGT_TGN_SocialViral(nn.Module):
    def __init__(self, num_nodes, node_feat_dim, memory_dim=24, message_dim=32, scn_dim=12, num_gt_layers=1,
                 update_memory_in_eval=True, edge_feat_dim=3):
        super().__init__()
        ...
        self.gt_layers = nn.ModuleList([
            # [수정] dropout=0.5
            SourceBasedGraphTransformerLayer(memory_dim, heads=4, ffn_dim=64, dropout=0.5)
            for _ in range(num_gt_layers)
        ])
        # [수정] dropout=0.5, edge_drop=0.2
        self.scn = SourceBasedSCNLayer(memory_dim, scn_dim, dropout=0.5, edge_drop=0.2)

        # [유지] 병렬 구조 (GT+SCN)
        self.edge_pred = nn.Sequential(
            nn.Linear(2 * (memory_dim + scn_dim), 32),
            nn.ReLU(),
            nn.Dropout(0.5), # [수정] 0.4 -> 0.5
            nn.Linear(32, 16),
            nn.ReLU(),
            nn.Dropout(0.5), # [수정] 0.4 -> 0.5
            nn.Linear(16, 1)
    ...
    def forward(self, src, dst, edge_feat, timestamps, node_features, adj):
        ...
        node_embs = self.node_emb(node_features)
        all_nodes = self.all_nodes_buf.to(device)
        mem_snapshot = self.memory.get_memory(all_nodes).clone()

        x0 = torch.cat([node_embs, mem_snapshot], dim=-1)
        x0 = self.mem_proj(x0)

        gt_input = x0.unsqueeze(0).expand(batch_size, -1, -1)
        
        # 1. GT 레이어 통과
        gt_out = gt_input
        for gt_layer in self.gt_layers:
            gt_out = gt_layer(gt_out) # (B, N, memory_dim)
        ...
        # 3. GT에서 임베딩 추출
        src_emb_gt = gt_out[b_idx, src] # (B, memory_dim)
        dst_emb_gt = gt_out[b_idx, dst] # (B, memory_dim)

        # 4. SCN에서 임베딩 추출
        src_emb_scn = scn_out[b_idx, src] # (B, scn_dim)
        dst_emb_scn = scn_out[b_idx, dst] # (B, scn_dim)

        # 5. [병렬 결합]
        src_final = torch.cat([src_emb_gt, src_emb_scn], dim=-1) # (B, memory_dim + scn_dim)
        dst_final = torch.cat([dst_emb_gt, dst_emb_scn], dim=-1) # (B, memory_dim + scn_dim)

        # 6. 최종 예측
        logits = self.edge_pred(torch.cat([src_final, dst_final], dim=-1)).squeeze(-1)
...
# ------------------------------------------------------------
# 8. 시간순 학습 파이프라인 (누수 방지, BF16, NaN 가드) + [수정] 70/15/15
# ------------------------------------------------------------
def source_based_train_gt_tgn_social(dataset, node_features, num_nodes,
    ...
    train_end = int(0.7 * total_len) # 70%
    ...
    model = SourceBasedGT_TGN_SocialViral(
        ...
        edge_feat_dim=edge_feat_dim).to(device)
    ...
    # [수정] weight_decay 1e-3 -> 2e-3 (정규화 강화)
    ...
    for epoch in range(epochs):
        ...
        for src, dst, edge_feat, ts, labels in train_loader:
            ...
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=(device == 'cuda')):
                ...
                # [유지] TGN 메모리 L2 정규화
                mem_reg = (model.memory.memory ** 2).mean()

                # [유지] 3) 최종 손실 (mem_reg 추가)
                loss = bce_loss + 0.2 * pair_loss + 1e-4 * mem_reg
        ...
        if val_labels.size > 0:
            ...
            if val_auc > best_val_auc:
                ...
                no_improve = 0 
                torch.save(model.state_dict(), '28_best_source_gt_tgn_social_reeee_2000.pth')
            else:
                no_improve += 1 
...
def source_based_evaluate_and_save_results(model, dataset_subset, node_features, adj, past_df, future_df,
                                           device='cpu', val_batch_size=16, out_csv='28_source_gt_tgn_social_viral_results(reeee_2000).csv',  warmup_indices=None):
    # [수정] print문 "Validation subset" -> "subset" (범용)
    ...
    if warmup_indices is not None and len(warmup_indices) > 0:
        ...
        # [수정] warmup_loader가 비어있지 않을 때만 실행
        if len(warm_loader) > 0:
            with torch.no_grad():
    ...
    if all_labels.size > 0:
        ...
        # [수정] (Validation) -> (Evaluation)
        print(f"\n=== Source-based GT-TGN+SCN 모델 성능 ({out_csv}) ===")
...
def main():
    ...
    try:
        ...
        # 2) 노드 제한: 2000개 (past 기준), 시간창/학습 설정
        max_nodes = 2000
        time_windows = 5
        epochs = 100
        train_batch_size = 32
        ...
        pair_topk = 35  # [유지] 30 (최적 데이터셋)
        patience = 15   # [유지] 조기 종료 Paitence
        ...
        # [수정] 6) 검증 세트(15%) 평가 및 저장
        results_df = source_based_evaluate_and_save_results(
            model=model,
            dataset_subset=val_dataset, # (학습 함수에서 15%로 분할됨)
            node_features=node_features,
            adj=adj,
            past_df=past_df,
            future_df=future_df,
            device=device,
            val_batch_size=val_batch_size,
            out_csv='28_source_gt_tgn_social_viral_results(reeee_tuned_2000).csv', # [수정] 파일명 변경
            warmup_indices=train_indices  
        )
        ...
        # -----------------------------------------------------
        # [수정] 7) ✅ 테스트 단계 (15%) + [추가] 지표 CSV 저장
        # -----------------------------------------------------
        print("\n=== [추가] TEST 단계 시작 ===")
        
        order = np.argsort(dataset.timestamps)
        total_len = len(dataset)
        test_start = int(0.85 * total_len)  # 70+15 = 85% 지점
        
        test_indices = order[test_start:] # 시간순 정렬
        test_dataset = Subset(dataset, test_indices)

        test_results_df = source_based_evaluate_and_save_results(
            model=model,
            ...
            device=device,
            val_batch_size=val_batch_size,
            out_csv='28_reeee_gt_tgn_scn_test_tuned.csv', # [수정] 파일명 변경
            warmup_indices=train_indices # Test시 웜업은 Train(70)만 사용
        )

        print("\n=== TEST 단계 완료 ===")
        
        # [수정] 지표 계산, 출력, CSV 저장 로직 (참조 코드와 동일하게)
        if test_results_df is not None and not test_results_df.empty:
            ...
            pd.DataFrame([{
                ...
                "f1": f1,
                "auc": auc
            }]).to_csv("28_gt_tgn_scn_test_metrics_tuned.csv", index=False, encoding="utf-8-sig") # [수정] 파일명 변경
            print("Saved: 28_gt_tgn_scn_test_metrics_tuned.csv")
        else:
            print("\n[Test] 평가용 샘플이 없습니다.")
