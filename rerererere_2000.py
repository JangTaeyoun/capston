    ...
    for epoch in range(epochs):
        ...
        if val_labels.size > 0:
            ...
            if val_auc > best_val_auc:
                ...
                torch.save(model.state_dict(), 'best_source_gt_tgn_social_rerererere_2000.pth')
...
def source_based_evaluate_and_save_results(model, dataset_subset, node_features, adj, past_df, future_df,
                                           device='cpu', val_batch_size=16, out_csv='source_gt_tgn_social_viral_results(rererere_2000).csv',  warmup_indices=None):
...
def main():
    ...
    try:
        ...
        results_df = source_based_evaluate_and_save_results(
            ...
            device=device,
            val_batch_size=val_batch_size,
            out_csv='source_gt_tgn_social_viral_results(rerererere_2000).csv',
        ...
        print("\n=== [추가] TEST 단계 시작 ===")
        total_len = len(dataset)
        test_start = int(0.9 * total_len)  # 전체의 뒤쪽 10%를 테스트로 사용
        test_indices = np.arange(test_start, total_len)
        ...
        test_results_df = source_based_evaluate_and_save_results(
            ...
            device=device,
            val_batch_size=val_batch_size,
            out_csv='gt_tgn_scn_test.csv',
            warmup_indices=train_indices
        )

        print("\n=== TEST 단계 완료 ===")
        print(f"테스트 엣지 수: {len(test_results_df)}")
        if not test_results_df.empty:
            acc = accuracy_score(test_results_df['actual_viral'], test_results_df['predicted_viral'])
            print(f"테스트 Accuracy: {acc*100:.2f}%")
            print(f"테스트 AUC: {roc_auc_score(test_results_df['actual_viral'], test_results_df['prediction_probability']):.4f}")
